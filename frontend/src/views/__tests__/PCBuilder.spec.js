...
describe('PCBuilder.vue', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    window.alert = vi.fn(); // Mock alerts
    ...
    builderStore.removePart.mockImplementation((stepId) => {
        delete builderStore.selectedParts[stepId];
    });
  });
  ...
  it('selects a component when clicked', async () => {
    ...
    expect(builderStore.selectPart).toHaveBeenCalledWith('cpu', expect.anything());
  });

  it('removes a selected part via sidebar', async () => {
    // 1. Simulate a selected part
    builderStore.selectedPartsArray = [{ id: 'cpu', label: 'CPU', component: { name: 'Intel i9' } }];
    await wrapper.vm.$nextTick();

    // 2. Find the red remove button (using class selector from your template)
  ...
  describe('Save/Buy Flows', () => {
    ...
    it('opens modal and saves successfully', async () => {
      authStore.token = 'valid-token';
      builderStore.allPartsSelected = true;
      await wrapper.vm.$nextTick();

      // Click Sidebar Save
      const sidebarSaveBtn = wrapper.findAllComponents(UIStubs.Button)
          .find(b => b.text().includes('Save Build'));
      await sidebarSaveBtn.trigger('click');
      
      expect(wrapper.find('.fixed').exists()).toBe(true); // Modal Open
      ...
      // Click Modal Save
      const modalSaveBtn = wrapper.findAllComponents(UIStubs.Button)
    ...
    it('shows alert if save fails (API Error)', async () => {
        // Mock Failure
        builderStore.saveBuild.mockResolvedValue({ success: false, error: 'DB Error' });
        authStore.token = 'valid-token';
        
        wrapper.vm.showSaveModal = true;
        wrapper.vm.buildName = 'Bad Build';
        await wrapper.vm.$nextTick();
    ...
    it('Buy Now: saves then transfers to cart', async () => {
      authStore.token = 'valid-token';
      builderStore.allPartsSelected = true;
